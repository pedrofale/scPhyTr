...
```math
\mathcal Q(\eta\mid\eta^{\text{old}}) \;=\; \mathbb E_{Z\sim p(\cdot\mid Y,\eta^{\text{old}})}\!\big[\log p(Y,Z\mid\eta)\big], \quad (1.1)
```
...
+ \sum_{u}\log\mathcal N\!\big(z_u - \phi_u z_{\mathrm{pa}(u)} - c_u;\ 0,\ v_u K\big), \quad (1.2)
```
...
\Sigma_{i,\mathrm{pa}} = \operatorname{Cov}(z_i, z_{\mathrm{pa}(i)}), \quad (1.3)
```
...
```math
p(Z\mid Y,\eta) \;\propto\; \exp\!\Big\{\underbrace{\ell(F)}_{\log p(Y\mid Z)} \;-\; \tfrac12 (Z-m)^\top Q\,(Z-m)\Big\}, \quad (2.1)
```
...
```math
\boxed{\ q_{\text{Lap}}(Z) = \mathcal N\!\big(Z;\ \hat Z,\ H^{-1}\big),\qquad H = Q + W.\ } \quad (3.1)
```
...
\Sigma_{ii} = d_i^{-1} + G_i\,\Sigma_{\mathrm{pa}}\,G_i^\top. \quad (3.2)
```
...
```math
\log w_s = \underbrace{\ell(F_s)}_{\text{decoder}} \;-\; \tfrac12\,(Z_s-m)^\top Q\,(Z_s-m) \;+\; \tfrac12\,(Z_s-\hat Z)^\top (Q+W)\,(Z_s-\hat Z) \;+\; \text{const}, \quad (5.1)
```
...
```math
\widehat{\log p(Y\mid\eta)} = \operatorname{logsumexp}_s(\log w_s^{\text{full}}) - \log S, \quad (5.2)
```
...
```math
\text{ESS} = \frac{1}{\sum_s \tilde w_s^2} \in [1, S]. \quad (5.3)
```
...
```math
\nabla_Z \log\pi(Z) = \nabla\ell(F)\big|_{\text{scattered to leaves}} - Q\,(Z-m), \quad (6.1)
```
