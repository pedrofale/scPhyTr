...
9. [Model selection: distinguishing drift from selection](#9-model-selection-distinguishing-drift-from-selection)
10. [Related work: LORACs, TreeVAE, SCOUT, VOUS](#10-related-work-loracs-treevae-scout-vous)
...
p(Y\mid\eta) \;=\; \int p(Y\mid Z)\,p(Z\mid\eta)\, dZ .
\tag{1.1}
$$
...
$$
\operatorname{vec}(Y_{\text{leaves}}) \sim \mathcal N\!\big(\mathbf 1\otimes\mu,\ C\otimes K\big).
\tag{2.1}
$$
...
\qquad v(t)=\frac{1-e^{-2\alpha t}}{2\alpha}.
\tag{2.2}
$$
...
$$
\mathbb E[z_i] = a\,e^{-\alpha T_i} + \theta\,(1-e^{-\alpha T_i}).
\tag{2.3}
$$
...
$$
Q_{uw}=0 \iff z_u \perp\!\!\!\perp z_w \mid \text{rest}.
\tag{4.1}
$$
...
- **Internal node:** push each child's belief up its branch, which *adds the branch length to the variance factor*, $v_c \mathrel{+}= t_c$. Then **fuse** the children. Fusing $\mathcal N(m_1,v_1K)$ and $\mathcal N(m_2,v_2K)$ that describe the same value:
  - emits a contrast $d=m_1-m_2 \sim \mathcal N(0,\,wK)$, $w=v_1+v_2$, contributing
    $$
    \log\mathcal N(d;0,wK) = -\tfrac12\Big[p\log2\pi + p\log w + \log|K| + \tfrac1w\,d^\top K^{-1}d\Big];
    \tag{5.1}
    $$
...
$$
\log p(Y\mid\eta)\approx \ell(\hat Z) + \log\mathcal N(\hat Z;m,\Sigma) + \tfrac N2\log2\pi - \tfrac12\log|H|.
\tag{6.1}
$$
...
$$
\boxed{\ \log p(Y\mid\eta)\ \approx\ \ell(\hat Z)\ -\ \tfrac12(\hat Z-m)^\top Q\,(\hat Z-m)\ +\ \tfrac12\log|Q|\ -\ \tfrac12\log|Q+W|\ }
\tag{6.2}
$$
...
$$
\hat K = \frac1n\sum_{i=1}^{n} u_i u_i^\top,
\tag{8.1}
$$
...
\Delta z_u = z_u - \phi_u z_{\mathrm{pa}(u)} - (1-\phi_u)\theta_{\rho(u)} .
\tag{8.2}
$$
...
\Sigma_{uu}=d_u^{-1}+G_u\,\Sigma_{\mathrm{pa}\,\mathrm{pa}}\,G_u^\top .
\tag{8.3}
$$
...
$$
\mathcal Q(\alpha,\theta,K)=\sum_u -\tfrac12\Big[\,p\log v_u + \log|K| + \tfrac1{v_u}\operatorname{tr}\!\big(K^{-1}M_u\big)\Big],
\tag{8.4}
$$
...
| Linear-time likelihoods (§5) | `utils/pruning.py` | `bm_pruning_logpdf`, `ou_pruning_logpdf`, `paint_regimes`, `_ou_branch`, `_gaussian_contrast_logpdf` |
